...
flightwake v0.5.0,內部測試中(kaiwutech-TW private),準備開源。缺口 1–5 已落地(去識別化、superseded 時效管理、多平台安裝、--private 本機模式、uninstall),冷啟動實測首筆有效樣本已入 benchmarks(量測工具 tools/session-cost.mjs,dev-only)。剩缺口 6–7。定位已確立:給強模型(Fable 5 級)的事後記錄框架,刻意不做 GSD 式導航——強模型自己會開車。
...
1. 缺口 6 CI 端 STATE 落後檢查 → 把 Stop hook 的紀律帶到 Claude Code 之外(可重用 hooks/state-check.mjs 的 rev-list 邏輯)
2. 缺口 7 monorepo 政策 → 單 repo 一份或允許 workspace 子目錄各裝,開源前想清楚
