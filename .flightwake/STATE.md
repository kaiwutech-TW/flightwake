...
flightwake v0.6.0,內部測試中(kaiwutech-TW private),準備開源。缺口 1–6 已落地(去識別化、superseded 時效管理、多平台安裝、--private 本機模式、uninstall、CI 端 STATE 檢查),冷啟動實測首筆有效樣本已入 benchmarks(量測工具 tools/session-cost.mjs,dev-only)。剩缺口 7(monorepo)。定位已確立:給強模型(Fable 5 級)的事後記錄框架,刻意不做 GSD 式導航——強模型自己會開車。
...
1. 缺口 7 monorepo 政策 → 單 repo 一份或允許 workspace 子目錄各裝,開源前想清楚(這是政策決策,先討論再動手)
2. 開源前 checklist 收殘:伺服器端舊 objects 清理、英文 README、npm Trusted Publishing(見 docs/design.md)
