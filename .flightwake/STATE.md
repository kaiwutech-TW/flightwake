...
flightwake v0.4.0,內部測試中(kaiwutech-TW private),準備開源。缺口 1–4 已落地(去識別化、superseded 時效管理、多平台安裝、--private 本機模式),冷啟動實測首筆有效樣本已入 benchmarks。剩缺口 5–7。定位已確立:給強模型(Fable 5 級)的事後記錄框架,刻意不做 GSD 式導航——強模型自己會開車。
...
1. 缺口 5 uninstall → `bin/cli.mjs` 加子指令(寫入範圍固定,反向刪除 + 移除三處標記區塊:指令檔 / CLAUDE.local.md / .git/info/exclude)
2. 缺口 6 CI 端 STATE 落後檢查 → 把 Stop hook 的紀律帶到 Claude Code 之外
