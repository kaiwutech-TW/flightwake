...
## Flip 日 runbook(開源當天,依序執行)

1. **推全新 repo**(伺服器端舊 objects 以舊 SHA 仍可存取,force push 不會清;全新 repo 最保險):
   新建 public `kaiwutech-TW/flightwake` 替身(或先聯絡 GitHub Support 跑 gc 再轉 public)→ `git push new-origin main`
2. ~~package.json private 刪除、README 對調~~ ✅ 已提前完成(2026-07-18:npm 佔位時刪 private;README 四語版 en 主版 + zh-TW/zh-CN/ja 互鏈)
3. npmjs.com 建立套件 trusted publisher 登記:repo `kaiwutech-TW/flightwake` + workflow `release.yml`(名字 `flightwake` 已確認未被占)
4. GitHub repo 設定:開 Private Vulnerability Reporting;branch protection(main 禁 force push)
5. 發第一個 GitHub Release(tag `v0.x.y`)→ release.yml 自動 smoke + `npm publish --provenance`
6. 轉 public 後確認 Scorecard workflow 跑起來、README 掛 badge:
   `https://api.scorecard.dev/projects/github.com/kaiwutech-TW/flightwake/badge`
7. 驗證:`npx flightwake init` 在乾淨 repo 實跑 + `npm audit signatures`
