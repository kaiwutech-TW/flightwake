...
- [ ] 開源準備:英文版 README、範例 repo、license(開源時同步把 package.json `private` 改回 false)
- [ ] 開源安全 checklist:
  - [x] 內部專案名/session id/內部路徑匿名化,含 git 歷史清除(v0.2)
  - [ ] 轉 public 前:GitHub 伺服器端舊 objects 仍可能以舊 SHA 存取(force push 不會清)——
    最保險是開源時推到**全新 repo**;或先聯絡 GitHub Support 跑 gc 再轉 public
  - [ ] npm Trusted Publishing(GitHub Actions OIDC,自動附 provenance;名字 `flightwake` 已確認未被占)
  - [ ] 轉 public 後確認 Scorecard workflow 跑起來、README 掛 badge:`https://api.scorecard.dev/projects/github.com/kaiwutech-TW/flightwake/badge`
  - [ ] GitHub 開 Private Vulnerability Reporting + branch protection(main 禁 force push)
