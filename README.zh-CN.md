...
health 颜色(你唯一要盯的事)、STATE 落后量(与 Stop hook 同一套 rev-list 逻辑,但从「结束时提醒」变成「随时看得到」)、context 用量——bar 烧红就是该收尾或交接的信号。绝不覆盖既有 statusline(单值设置),且 repo 层设置优先于用户层,与全局仪表工具可并存。
