...
health の色(あなたが見張る唯一のもの)、STATE の遅れ(Stop hook と同じ rev-list ロジックを「終了時の催促」から「常時表示」に)、context 使用量——バーが赤くなったら締めるか引き継ぐかの合図。既存の statusline は決して上書きせず(単一値設定)、repo 層の設定がユーザー層より優先されるため、グローバルなゲージツールとも共存できる。
