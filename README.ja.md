...
締め忘れても大丈夫:STATE が 3 コミット以上遅れると、Stop hook がセッション終了前に一度ブロックして催促する(STATE が health=green を主張しているのに最新 record にテスト証拠が無いときも催促する);`--ci` で同じゲートを他の agent や人間の協力者にも届けられる。このネットの限界も正直に:遅れは**人間の**コミットだけを数える——bot のコミット(`dependabot[bot]`、`renovate[bot]` など)は除外される。依存の更新が STATE を誤らせることはなく、bot 自身の PR ではこのゲートを満たしようがないからだ。コミットを残さないセッション(調査、ops 作業)や squash/rebase フローはネットの下をすり抜ける——ネットが受け止めるのは「忘れ」であり、セッション終了時の義務の代わりではない。セッションをまたぐ大きな工事は、手を止める前に「引き継ぎ」と言って `/fw-handoff` を走らせる。
...
**Claude Code のメモリ機能**:永続メモリは flightwake と同形(frontmatter + `[[リンク]]`)だが別レイヤー——メモリは単一マシン・単一人;flightwake のファイルは git に入り、repo と共にチーム・CI・あらゆる agent へ届く。分担ルール:repo の事実(罠、決定、状態)は flightwake へ;個人の好みやプロジェクト横断の習慣はメモリへ。同じ事実を両方に書かない——ただし意図的な例外がひとつ:**この repo 固有ではない汎用の罠**(プラットフォーム/言語レイヤー)は両方に保存する。repo の登録簿は自己完結でなければならず、他の repo にも同じ警告が必要だから(スコープごとに一部ずつは分業であり重複ではない)。
