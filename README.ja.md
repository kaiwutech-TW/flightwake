...
締め忘れても大丈夫:STATE が 3 コミット以上遅れると、Stop hook がセッション終了前に一度ブロックして催促する(STATE が health=green を主張しているのに最新 record にテスト証拠が無いときも催促する);`--ci` で同じゲートを他の agent や人間の協力者にも届けられる。このネットの限界も正直に:遅れはコミット数で数えるため、コミットを残さないセッション(調査、ops 作業)や squash/rebase フローはネットの下をすり抜ける——ネットが受け止めるのは「忘れ」であり、セッション終了時の義務の代わりではない。セッションをまたぐ大きな工事は、手を止める前に「引き継ぎ」と言って `/fw-handoff` を走らせる。
